...
productName="Toggler"
VsSolutionFile = "FeatureToggles.sln"
workingDir=Dir.pwd
...
nunitconsole="packages/NUnit.Runners.2.6.2/tools/nunit-console.exe"
nugetPath= 'packages_manual/nuget.exe'
zipExe = "packages_manual/7zip/7z.exe"
os_platform="x64"

task :default => :build
task :build => [ :clean, :envConfig, :compile, :runTests]
task :commitBuild => [ :clean, :compile, :runTests, :codeAnalysis]
...
task :runTests do
	if (ENV['TEAMCITY_JRE'] !=nil) then	
		...
		testAssemblies = Dir.glob("buildArtifacts/bin/**/*tests*.dll")
